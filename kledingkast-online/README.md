...
> **Dit is de werkmap voor de serverversie.** Hij begint als een exacte kopie van
> `kledingkast/`, met eigen opslagnamen zodat de twee elkaar niet in de weg
> zitten. Wat er moet gebeuren om hier een echte, deelbare app van te maken staat
> in **[PLAN.md](PLAN.md)**. Zolang daar nog niets van gebouwd is, werkt deze map
> precies zoals de lokale app hieronder beschreven — dus zonder server.
>
> De werkende lokale app blijft in `kledingkast/` staan. Wijzig die niet vanuit
> hier; vanaf nu lopen ze uit elkaar.
